...
export const MatchScoreControls = ({ data, onChange, accentColor = "orange" }) => (
  <div className={`bg-${accentColor}-50 p-3 rounded-xl border border-${accentColor}-100 mb-4`}>
    <label className={`text-xs font-bold text-${accentColor}-800 uppercase mb-2 block text-center`}>Punteggio</label>
    <div className="grid grid-cols-2 gap-4">
      <input
        type="number"
        value={data.homeScore}
        onChange={(e) => onChange('homeScore', e.target.value)}
        className={`p-3 text-center text-2xl font-black border-2 border-white rounded-xl shadow-sm focus:border-${accentColor}-500`}
        placeholder="0"
      />
      <input
        type="number"
